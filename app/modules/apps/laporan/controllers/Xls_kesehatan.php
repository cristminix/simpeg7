...
class Xls_kesehatan extends MX_Controller {
...
//name the worksheet
$this->myexcel->getActiveSheet()->setTitle('Laporan Kesehatan');
//set cell A1 content with some text
$this->myexcel->getActiveSheet()->setCellValue('B1', 'Laporan Kesehatan Pegawai');
...
$this->myexcel->getActiveSheet()->getColumnDimension("G")->setWidth(150);
...
$this->myexcel->getActiveSheet()->setCellValue('E'.$rc, 'TGL TES');
$this->myexcel->getActiveSheet()->getStyle('E'.$rc)->applyFromArray($styleArray3);
$this->myexcel->getActiveSheet()->setCellValue('F'.$rc, 'TEMPAT');
$this->myexcel->getActiveSheet()->getStyle('F'.$rc)->applyFromArray($styleArray3);
$this->myexcel->getActiveSheet()->setCellValue('G'.$rc, 'HASIL');
...
$styleArray7 = array(
      'borders' => array(
        ...
        'bottom' => array('style' => PHPExcel_Style_Border::BORDER_HAIR,), 
			...
			$batas = $iptHal1;
			$start = ($awal-1)*$batas;
			$hsl = $this->m_lap->get_lap_kesehatan($iptHal1,$iptHal2,$tgl1,$tgl2);
...
foreach($hsl as $key=>$val){
	...
	$tanggal_tes=  " ".date("d-m-Y", strtotime($val->tanggal_tes));
	$this->myexcel->getActiveSheet()->setCellValue('E'.$rc, $tanggal_tes);
...
///////////////////////////////////////////////////////////////////////////
$filename='daftar_kesehatan_pegawai.xls'; //save our workbook as this file name

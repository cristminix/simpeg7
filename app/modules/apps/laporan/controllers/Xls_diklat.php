...
class Xls_diklat extends MX_Controller {
...
//name the worksheet
$this->myexcel->getActiveSheet()->setTitle('Laporan Diklat');
//set cell A1 content with some text
$this->myexcel->getActiveSheet()->setCellValue('B1', 'Laporan Diklat Pegawai');
...
$this->myexcel->getActiveSheet()->getColumnDimension("F")->setWidth(40	);
$this->myexcel->getActiveSheet()->getColumnDimension("G")->setWidth(13);
$this->myexcel->getActiveSheet()->getColumnDimension("H")->setWidth(13);
$this->myexcel->getActiveSheet()->getColumnDimension("I")->setWidth(25);
$this->myexcel->getActiveSheet()->getColumnDimension("J")->setWidth(13);
...
$styleArray7 = array(
      'borders' => array(
        ...
        'bottom' => array('style' => PHPExcel_Style_Border::BORDER_HAIR,), 
			...
			$batas = $iptHal1;
			$start = ($awal-1)*$batas;
			$hsl = $this->m_lap->get_lap_diklat($iptHal1,$iptHal2,$tgl1,$tgl2);
...
foreach($hsl as $key=>$val){
	...
	$this->myexcel->getActiveSheet()->setCellValue('E'.$rc, $nama_diklat);
	$this->myexcel->getActiveSheet()->getStyle('E'.$rc)->applyFromArray($styleArray6);
	$tempat_diklat=$val->tempat_diklat;
	$this->myexcel->getActiveSheet()->setCellValue('F'.$rc, $tempat_diklat);
	$this->myexcel->getActiveSheet()->getStyle('F'.$rc)->applyFromArray($styleArray6);
	$tanggal_mulai=" ".date("d-m-Y", strtotime($val->tanggal_mulai));
	$this->myexcel->getActiveSheet()->setCellValue('G'.$rc, $tanggal_mulai);
	$this->myexcel->getActiveSheet()->getStyle('G'.$rc)->applyFromArray($styleArray6);
	$tanggal_selesai=" ".date("d-m-Y", strtotime($val->tanggal_selesai));
	$this->myexcel->getActiveSheet()->setCellValue('H'.$rc, $tanggal_selesai);
	$this->myexcel->getActiveSheet()->getStyle('H'.$rc)->applyFromArray($styleArray6);
	$nomor_sk=$val->nomor_sk;
	$this->myexcel->getActiveSheet()->setCellValue('I'.$rc, $nomor_sk);
	$this->myexcel->getActiveSheet()->getStyle('I'.$rc)->applyFromArray($styleArray6);
	$tanggal_sk=" ".date("d-m-Y", strtotime($val->tanggal_sk));
	$this->myexcel->getActiveSheet()->setCellValue('J'.$rc, $tanggal_sk);
...
///////////////////////////////////////////////////////////////////////////
$filename='daftar_diklat_pegawai.xls'; //save our workbook as this file name

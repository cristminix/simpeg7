...
class Xls_jabatan extends MX_Controller {
...
//name the worksheet
$this->myexcel->getActiveSheet()->setTitle('Laporan Jabatan');
//set cell A1 content with some text
$this->myexcel->getActiveSheet()->setCellValue('B1', 'Laporan Jabatan Pegawai');
...
$this->myexcel->getActiveSheet()->getColumnDimension("G")->setWidth(50);
$this->myexcel->getActiveSheet()->getColumnDimension("H")->setWidth(50);
...
$this->myexcel->getActiveSheet()->setCellValue('H'.$rc, 'UNIT KERJA');
...
$this->myexcel->getActiveSheet()->getStyle('B4:H4')->applyFromArray($styleArray);
$this->myexcel->getActiveSheet()->getStyle('B1')->getFont()->setBold(true);
...
$styleArray7 = array(
      'borders' => array(
        ...
        'bottom' => array('style' => PHPExcel_Style_Border::BORDER_HAIR,), 
			...
			// $start = ($awal-1)*$batas;
			// $hsl = $this->m_lap->get_lap_jabatan($cari,$start,$batas); 
			
			$id_print=$this->session->userdata('id_cetak');
			$cari = $id_print['cari'];
			$batas = $iptHal1;
			$start = ($awal-1)*$batas;
			$hsl = $this->m_lap->get_lap_jabatan($iptHal1,$iptHal2,$tgl1,$tgl2);
...
foreach($hsl as $key=>$val){
	...
	$tanggal_sk = " ".date("d-m-Y", strtotime($val->sk_tanggal));
	...
	$unor=" ".$val->nama_unor;
	$this->myexcel->getActiveSheet()->setCellValue('H'.$rc, $unor);
	$this->myexcel->getActiveSheet()->getStyle('H'.$rc)->applyFromArray($styleArray7);
...
$this->myexcel->getActiveSheet()->getStyle('B'.$rc.':H'.$rc)->applyFromArray($styleArray9);
$this->myexcel->getActiveSheet()->getStyle('H'.$rc)->applyFromArray($styleArray10);

///////////////////////////////////////////////////////////////////////////
$filename='daftar_jabatan_pegawai.xls'; //save our workbook as this file name

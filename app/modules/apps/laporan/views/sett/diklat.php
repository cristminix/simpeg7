<div class="row">
	<div class="col-lg-12">
		 <h3 class="page-header">Laporan Diklat Pegawai</h3>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
		...
		<div class="panel panel-default">
			<div class="panel-heading">
						<div class="row">
								...
								<div style="float:left; padding-top:3px;padding-left:5px;"> </div>
								<div style="float:left; padding-top:3px;padding-left:5px;"> </div>
								<div style="float:left; padding-top:3px;padding-left:5px;">Tgl SK : </div>
								<div class="container">
...
<thead id=gridhead>
	<tr height=20>
		...
		<th style="width:150px;" align=center>NAMA PEGAWAI</th>
		<th style="width:80px;" align=center>NIP</th>
		<th style="width:150px;" align=center>NAMA DIKLAT</th>
		<th style="width:150px;" align=center>TEMPAT DIKLAT</th>
		<th style="width:200px;" align=center>DIKLAT</th>
	</tr>
</thead>
<tbody id=list>
...
var kode_unor = $('#kode_unor').val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>laporan/sett/get_diklat/",
...
if((data.hslquery.length)>0){
			...
			$.each( data.hslquery, function(index, item){
				...
				table = table+ '<td>';
				table = table+ '<div>';
				table = table+ '<span><div style="display:table;">'+item.nama_diklat+'</div></span>';
				table = table+ '</div></td>';
				table = table+ '<td>';
				table = table+ '<div>';
				table = table+ '<span><div style="display:table;">'+item.tempat_diklat+'</div></span>';
				table = table+ '</div></td>';
				
				table = table+ '<td>';
				table = table+ '<div>';
				table = table+ '<div style="float:left; width:65px;">Tgl.  Mulai</div>';
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span><div style="display:table;">'+item.tanggal_mulai+'</div></span>';
				table = table+ '</div>';
				table = table+ '<div>';
				table = table+ '<div style="float:left; width:65px;">Tgl. Sls</div>';
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span><div style="display:table;">'+item.tanggal_selesai+'</div></span>';
				table = table+ '</div>';
				table = table+ '<div>';
				table = table+ '<div style="float:left; width:65px;">No. SK</div>';
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span><div style="display:table;">'+item.nomor_sk+'</div></span>';
				table = table+ '</div>';
				table = table+ '<div>';
				table = table+ '<div style="float:left; width:65px;">Tgl. SK</div>';
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span>'+item.tanggal_sk+'</span>';
				table = table+ '</div>';
				table = table+ '</td>';
...
function cetak_excel(iptHal1,iptHal2,tgl1,tgl2){
			if ((iptHal1 == '') && (iptHal2!='')){
				...
				var val1='0';
				var val2='0';
				window.open("<?=site_url();?>laporan/xls_diklat/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");
			}else{
				var val1=iptHal1;
				var val2=iptHal2;
				window.open("<?=site_url();?>laporan/xls_diklat/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");

<div class="row">
	<div class="col-lg-12">
		 <h3 class="page-header">Laporan Jabatan Pegawai</h3>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
		...
		<div class="panel panel-default">
			<div class="panel-heading">
							<div class="row">
								...
								<div style="float:left; padding-top:3px;padding-left:5px;"> </div>
								<div style="float:left; padding-top:3px;padding-left:5px;"> </div>
								<div style="float:left; padding-top:3px;padding-left:5px;">Tmt Jabatan : </div>
								<div class="container">
...
var kode_unor = $('#kode_unor').val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>laporan/sett/get_jabatan/",
...
if((data.hslquery.length)>0){
			...
			$.each( data.hslquery, function(index, item){
				...
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span>'+item.nama_jabatan+'</span>';
				table = table+ '</div>';
				table = table+ '<div style="clear:both">';
				table = table+ '<div style="float:left; width:65px;">Unit Kerja</div>';
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span>'+item.nama_unor+'</span>';
				table = table+ '</div>';
...
function cetak_excel(iptHal1,iptHal2,tgl1,tgl2){
			if ((iptHal1 == '') && (iptHal2!='')){
				...
				var val1='0';
				var val2='0';
				window.open("<?=site_url();?>laporan/xls_jabatan/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");
			}else{
				var val1=iptHal1;
				var val2=iptHal2;
				window.open("<?=site_url();?>laporan/xls_jabatan/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");	

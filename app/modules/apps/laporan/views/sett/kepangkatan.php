<div class="row">
	<div class="col-lg-12">
		 <h3 class="page-header">Laporan Kepangkatan Pegawai</h3>
	</div>
	<!-- /.col-lg-12 -->
</div>
		...
		<div class="panel panel-default">
			<div class="panel-heading">
						<div class="row">
								...
								<div style="float:left; padding-top:3px;padding-left:5px;"> </div>
								<div style="float:left; padding-top:3px;padding-left:5px;"> </div>
								<div style="float:left; padding-top:3px;padding-left:5px;">Tmt Pangkat : </div>
								<div class="container">
...
<thead id=gridhead>
	<tr height=20>
		...
		<th style="width:90px;" align=center>NIP</th>
		<th style="width:250px;" align=center>KEPANGKATAN</th>
	</tr>
</thead>
<tbody id=list>
...
var kode_unor = $('#kode_unor').val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>laporan/sett/get_kepangkatan/",
...
if((data.hslquery.length)>0){
			...
			$.each( data.hslquery, function(index, item){
				...
				table = table+ '<span><div style="display:table;">'+item.sk_nomor+'</div></span>';
				table = table+ '</div>';
				table = table+ '<div style="clear:both">';
				table = table+ '<div style="float:left; width:65px;">Tgl. SK</div>';
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span>'+item.sk_tanggal+'</span>';
				table = table+ '</div>';
				table = table+ '<div style="clear:both">';
				table = table+ '<div style="float:left; width:65px;">Pangkat</div>';
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span>'+item.nama_pangkat+'</span>';
				table = table+ '</div>';
				table = table+ '<div style="clear:both">';
				table = table+ '<div style="float:left; width:65px;">Golongan</div>';
				table = table+ '<div style="float:left; width:10px;">:</div>';
				table = table+ '<span>'+item.nama_golongan+'</span>';
				table = table+ '</div>';
...
function cetak_excel(iptHal1,iptHal2,tgl1,tgl2){
			...
			if ((iptHal1 == '') && (iptHal2!='')){
				...
				var val1='0';
				var val2='0';
				window.open("<?=site_url();?>laporan/xls_pangkat/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");
			}else{
				var val1=iptHal1;
				var val2=iptHal2;
				window.open("<?=site_url();?>laporan/xls_pangkat/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");

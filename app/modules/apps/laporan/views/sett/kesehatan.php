<div class="row">
	<div class="col-lg-12">
		 <h3 class="page-header">Laporan Kesehatan Pegawai</h3>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
		...
		<div class="panel panel-default">
			<div class="panel-heading">
						<div class="row">
								...
								<div style="float:left; padding-top:3px;padding-left:5px;"> </div>
								<div style="float:left; padding-top:3px;padding-left:5px;"> </div>
								<div style="float:left; padding-top:3px;padding-left:5px;">Tgl Tes : </div>
								<div class="container">
...
<thead id=gridhead>
	<tr height=20>
		...
		<th style="width:80px;" align=center>TANGGAL TES</th>
		<th style="width:150px;" align=center>TEMPAT</th>
		<th style="width:250px;" align=center>HASIL</th>
	</tr>
</thead>
<tbody id=list>
...
var kode_unor = $('#kode_unor').val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>laporan/sett/get_kesehatan/",
...
if((data.hslquery.length)>0){
			...
			$.each( data.hslquery, function(index, item){
				...
				table = table+ '<td>';
				table = table+ '<div>';
				table = table+ '<span><div style="display:table;">'+item.tanggal_tes+'</div></span>';
				table = table+ '</div></td>';
				table = table+ '<td>';
				table = table+ '<div>';
				table = table+ '<span><div style="display:table;">'+item.tempat+'</div></span>';
				table = table+ '</div></td>';
				table = table+ '<td>';
				table = table+ '<div>';
				table = table+ '<span><div style="display:table;">'+item.hasil+'</div></span>';
				table = table+ '</div></td>';
...
function cetak_excel(iptHal1,iptHal2,tgl1,tgl2){
			if ((iptHal1 == '') && (iptHal2!='')){
				...
				var val1='0';
				var val2='0';
				window.open("<?=site_url();?>laporan/xls_kesehatan/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");
			}else{
				var val1=iptHal1;
				var val2=iptHal2;
				window.open("<?=site_url();?>laporan/xls_kesehatan/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");

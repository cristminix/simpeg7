<div class="row">
	<div class="col-lg-12">
		 <h3 class="page-header">Laporan Penghargaan Pegawai</h3>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
...
<thead id=gridhead>
	<tr height=20>
		...
		<th style="width:150px;" align=center>NOMOR SK</th>
		<th style="width:90px;" align=center>TANGGAL SK</th>
		<th style="width:250px;" align=center>URAIAN</th>
	</tr>
</thead>
<tbody id=list>
...
var kode_unor = $('#kode_unor').val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>laporan/sett/get_penghargaan/",
...
if((data.hslquery.length)>0){
			...
			$.each( data.hslquery, function(index, item){
				...
				table = table+ '<td>';
				table = table+ '<div>';
				table = table+ '<span><div style="display:table;">'+item.tanggal_sk+'</div></span>';
				table = table+ '</div></td>';
				table = table+ '<td>';
				table = table+ '<div>';
				table = table+ '<span><div style="display:table;">'+item.uraian+'</div></span>';
				table = table+ '</div></td>';
...
function cetak_excel(iptHal1,iptHal2,tgl1,tgl2){
			if ((iptHal1 == '') && (iptHal2!='')){
				...
				var val1='0';
				var val2='0';
				window.open("<?=site_url();?>laporan/xls_penghargaan/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");
			}else{
				var val1=iptHal1;
				var val2=iptHal2;
				window.open("<?=site_url();?>laporan/xls_penghargaan/index/"+val1+"/"+val2+"/"+tgl1+"/"+tgl2,"_blank");

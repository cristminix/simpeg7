						...
						<div class="row">
								<div class="col-lg-6">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-list fa-fw"></span></button>
										...
										<span id="judul">Master Gaji Pokok</span>
									</div>
								</div>
								<div class="col-lg-6">
									<div class="btn btn-primary btn-xs" style="float:right;" onClick="setForm('tambah','xx');"><i class="fa fa-plus fa-fw"></i> Tambah Master Gaji Pokok</div>
								</div>
						</div>
		</div>
...
<thead>
<tr>
<th style="width:10px;text-align:center; vertical-align:middle">No.</th>
<th style="width:40px;text-align:center; vertical-align:middle;padding:0px;">AKSI</th>
<th style="width:105px;text-align:center; vertical-align:middle">PANGKAT</th>
<th style="width:55px;text-align:center; vertical-align:middle">MASA KERJA</th>
<th style="width:85px;text-align:center; vertical-align:center">GAJI POKOK</th>
<th style="width:65px;text-align:center; vertical-align:middle">TAHUN</th>
<th style="width:65px;text-align:center; vertical-align:middle">STATUS</th>
</tr>
</thead>
<tbody id=list>
</tbody>
...
var tipe = $('#tipe').html();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>apppenggajian/gaji_pokok/getdata",
		...
		success:function(data){
			if((data.hslquery.length)>0){
				...
				$.each( data.hslquery, function(index, item){
					...
					table = table+ "<td style='padding:3px;' align='center'>"+no+"</td>";
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'edit\',\''+item.id_gaji_pokok+'\');"><i class="fa fa-edit fa-fw"></i> Edit data</a></li>';
						table = table+ '<li role="presentation" class="divider"></li>';
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'hapus\',\''+item.id_gaji_pokok +'\');"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>';
						table = table+ "</ul></div>";
					table = table+ "</td>";
	//tombol aksi<--
					table = table+ "<td style='padding:3px;'>"+item.nama_pangkat+"</td>";
					table = table+ "<td style='padding:3px;' align='center'>"+item.masa_kerja+"</td>";
					table = table+ "<td style='padding:3px;' align='right'>"+ formatNumber(item.gaji_pokok) +"</td>";
					table = table+ "<td style='padding:3px;' align='center'>"+item.tahun+"</td>";
					table = table+ "<td style='padding:3px;' align='center'>"+item.status+"</td>";
					table = table+ "</tr>";
					no++;
				}); //endeach
					$('#list').html(table);
...
var hal=$("#inputpaging").val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>apppenggajian/gaji_pokok/form"+aksi,

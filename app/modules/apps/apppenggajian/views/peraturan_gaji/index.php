						...
						<div class="row">
								<div class="col-lg-6">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-list fa-fw"></span></button>
										...
										<span id="judul">Master Peraturan Gaji</span>
									</div>
								</div>
								<div class="col-lg-6">
									<div class="btn btn-primary btn-xs" style="float:right;" onClick="setForm('tambah','xx');"><i class="fa fa-plus fa-fw"></i> Tambah Master Peraturan Gaji</div>
								</div>
						</div>
		</div>
...
<th style="width:105px;text-align:center; vertical-align:middle">PERATURAN GAJI</th>
<th style="width:65px;text-align:center; vertical-align:middle">TAHUN</th>
<th style="width:65px;text-align:center; vertical-align:middle">FILE</th>
...
var tipe = $('#tipe').html();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>apppenggajian/peraturan_gaji/getdata",
		...
		success:function(data){
			if((data.hslquery.length)>0){
				...
				$.each( data.hslquery, function(index, item){
					table = table+ "<tr id='row_"+item.id_peraturan_gaji+"'>";
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'edit\',\''+item.id_peraturan_gaji+'\');"><i class="fa fa-edit fa-fw"></i> Edit data</a></li>';
						table = table+ '<li role="presentation" class="divider"></li>';
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'hapus\',\''+item.id_peraturan_gaji+'\');"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>';
						table = table+ "</ul></div>";
					table = table+ "</td>";
	//tombol aksi<--
					table = table+ "<td style='padding:3px;'>"+item.peraturan_gaji+"</td>";
					table = table+ "<td style='padding:3px;' align='center'>"+item.tahun+"</td>";
					table = table+ "<td style='padding:3px;' align='center'>"+item.file+"</td>";
					// table = table+ "<td style='padding:3px;' align='center'>"+'<a href="file:\\D:\cc.pdf">111.pdf</a>'+"</td>";
...
var hal=$("#inputpaging").val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>apppenggajian/peraturan_gaji/form"+aksi,

	...
	<div class="col-lg-12">
		<div class="panel panel-success">
			<div class="panel-heading">
						<div class="row">
								<div class="col-lg-6">
									<i class="fa fa-fire fa-fw"></i> <b>Daftar Pegawai Meninggal</b>
								</div>
								<!--//col-lg-6-->
								<div class="col-lg-6">
...
<button class="btn btn-primary" type="button" onclick="setSubForm('tambah','xx','xx');"><i class="fa fa-plus fa-fw"></i> Tambah pegawai meninggal</button>
</td>
</tr>
<tbody>
</table>
...
var batas = $('#item_lengthB').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/pegawai/getsub",
		data:{"hal": hal, "batas": batas,"sub":"meninggal","cari":cari,"kehal":"pagingB"},
		beforeSend:function(){	
	...
	//tombol aksi<--
					...
					table = table+ "<div>";
					table = table+ '<div style="float:left; width:130px;">Tempat meninggal</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					table = table+ '<div style="float:left;">'+item.tempat_meninggal+'</div>';
					table = table+ '</div>';
					table = table+ '<div style="clear:both;">';
					table = table+ '<div style="float:left; width:130px;">Tanggal meninggal</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					table = table+ '<div style="float:left;">'+item.tanggal_meninggal+'</div>';
					table = table+ '</div>';
					table = table+ '<div style="clear:both;">';
					table = table+ '<div style="float:left; width:130px;">Sebab meninggal</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					table = table+ '<div style="float:left;">'+item.sebab_meninggal+'</div>';
					table = table+ '</div>';
					table = table+ "</td>";
					table = table+ "</tr>";
					no++;
...
function setSubForm(aksi,idd,no){
	$('.btn.batal').click();
		$.ajax({
        type:"POST",
		url:"<?=site_url();?>appbkpp/pegawai/formsub_meninggal_"+aksi,
		data:{"idd": idd,"nomor":no,"sub":"meninggal" },
		beforeSend:function(){
        ...
        success:function(data){
			$('#form_sub').attr('action','<?=site_url("appbkpp/pegawai/formsub_meninggal_");?>'+aksi+'_aksi');
			$('#brow_'+idd).hide();

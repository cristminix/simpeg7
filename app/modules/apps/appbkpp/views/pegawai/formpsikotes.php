	...
	<div class="col-lg-12">
		<div class="panel panel-success">
			<div class="panel-heading">
						<div class="row">
								<div class="col-lg-6">
									<i class="fa fa-edit fa-fw"></i> <b>Form Update Riwayat Psikotes Pegawai</b>
								</div>
								<!--//col-lg-6-->
								<div class="col-lg-6">
	...
	<div class="col-lg-6">
		<div class="panel panel-default" style="margin-bottom:5px;" id="panel_pegawai">
			<div class="panel-heading">
				...
				<span id=judul_box_pegawai><b>RIWAYAT PSIKOTES PEGAWAI</b></span>
			</div>
...
<div class="row psikotes" id="grid-data">
	<div class="col-lg-12">
		<div class="table-responsive">
<form id="form_psikotes" method="post" enctype="multipart/form-data">
...
<table class="table table-striped table-bordered table-hover" style="width:1024px;" id="riwayat_psikotes">
<thead id=gridhead>
<tr>
...
<button class="btn btn-primary" type="button" onclick="setSubForm('tambah','xx',<?=$no;?>);"><i class="fa fa-plus fa-fw"></i> Tambah Riwayat Psikotes</button>
</td>
</tr>
<tbody>
</table>
...
function setSubForm(aksi,idd,no){
	$('.btn.batal').click();
		...
		$.ajax({
        type:"POST",
		url:"<?=site_url();?>appbkpp/pegawai/formpsikotes_"+aksi,
        ...
        success:function(data){
			$('#form_psikotes').attr('action','<?=site_url("appbkpp/pegawai/formpsikotes_");?>'+aksi+'_aksi');
			$('#row_'+idd).hide();
...
function baru(){
	$.ajax({
		type:"POST",
		url: "<?=site_url();?>appbkpp/pegawai/psikotes_riwayat",
		data: $("#form_psikotes").serialize(),
		beforeSend:function(){
			$('#riwayat_psikotes tbody').remove();
			$('<tbody><tr id="list_riwayat"><td colspan=7><p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p></td></tr></tbody>').insertAfter('#riwayat_psikotes thead');
		},
		success:function(data){
			...
			footer = footer +'<button class="btn btn-primary" type="button" onclick="setSubForm(\'tambah\',\'xx\','+data.no+');"><i class="fa fa-plus fa-fw"></i> Tambah riwayat psikotes</button>';
			footer = footer +'</td></tr>';
			var table = data.psikotes+footer;
...
function validasi_isian(){
	...
	var dati="";
			...
			var tanggal_tes = $.trim($("#tanggal_tes").val());
			...
			if( tanggal_tes ==""){	dati=dati+"TANGGAL TES tidak boleh kosong\n";	}

	...
	<div class="col-lg-12">
		<div class="panel panel-success">
			<div class="panel-heading">
						<div class="row">
								<div class="col-lg-6">
									<i class="fa fa-edit fa-fw"></i> <b>Form Update Pengalaman Pegawai</b>
								</div>
								<!--//col-lg-6-->
								<div class="col-lg-6">
	...
	<div class="col-lg-6">
		<div class="panel panel-default" style="margin-bottom:5px;" id="panel_pegawai">
			<div class="panel-heading">
				...
				<span id=judul_box_pegawai><b>PENGALAMAN PEGAWAI</b></span>
			</div>
...
<div class="row pengalaman" id="grid-data">
	<div class="col-lg-12">
		<div class="table-responsive">
<form id="form_pengalaman" method="post" enctype="multipart/form-data">
...
<table class="table table-striped table-bordered table-hover" style="width:1024px;" id="riwayat_pengalaman">
<thead id=gridhead>
<tr>
...
<th style="width:100px;text-align:center;vertical-align:middle;">PERUSAHAAN</th>
<th style="width:100px;text-align:center;vertical-align:middle;">PEKERJAAN</th>
<th style="width:50px;text-align:center;vertical-align:middle;">TANGGAL AWAL</th>
<th style="width:50px;text-align:center;vertical-align:middle;">TANGGAL AKHIR</th>
<th style="width:100px;text-align:center;vertical-align:middle;">JABATAN</th>
</tr>
</thead>
<tbody>
<?=$pengalaman;?>
...
<button class="btn btn-primary" type="button" onclick="setSubForm('tambah','xx',<?=$no;?>);"><i class="fa fa-plus fa-fw"></i> Tambah Pengalaman</button>
</td>
</tr>
<tbody>
</table>
...
function setSubForm(aksi,idd,no){
	$('.btn.batal').click();
		...
		$.ajax({
        type:"POST",
		url:"<?=site_url();?>appbkpp/pegawai/formpengalaman_"+aksi,
        ...
        success:function(data){
			$('#form_pengalaman').attr('action','<?=site_url("appbkpp/pegawai/formpengalaman_");?>'+aksi+'_aksi');
			$('#row_'+idd).hide();
...
function baru(){
	$.ajax({
		type:"POST",
		url: "<?=site_url();?>appbkpp/pegawai/pengalaman_riwayat",
		data: $("#form_pengalaman").serialize(),
		beforeSend:function(){
			$('#riwayat_pengalaman tbody').remove();
			$('<tbody><tr id="list_riwayat"><td colspan=7><p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p></td></tr></tbody>').insertAfter('#riwayat_pengalaman thead');
		},
		success:function(data){
			...
			footer = footer +'<button class="btn btn-primary" type="button" onclick="setSubForm(\'tambah\',\'xx\','+data.no+');"><i class="fa fa-plus fa-fw"></i> Tambah Pengalaman</button>';
			footer = footer +'</td></tr>';
			var table = data.pengalaman+footer;
...
//	if (hasil!=false) {
	$.ajax({
		type:"POST",
		url: $("#form_pengalaman").attr('action'),
		data: $("#form_pengalaman").serialize(),
		beforeSend:function(){	
			$('.bt_simpan').remove();
		},
...
function validasi_isian(){
	var data="";
	var dati="";
			var perusahaan = $.trim($("#perusahaan").val());
			var pekerjaan = $.trim($("#pekerjaan").val());
			var tanggal_awal = $.trim($("#tanggal_awal").val());
			var tanggal_akhir = $.trim($("#tanggal_akhir").val());
			var jabatan = $.trim($("#jabatan").val());
			data=data+""+iunr+"*"+tanggal_tes+"**";
			if( perusahaan ==""){	dati=dati+"PERUSAHAAN tidak boleh kosong\n";	}
			if( pekerjaan ==""){	dati=dati+"PEKERJAAN tidak boleh kosong\n";	}
			if( tanggal_awal ==""){	dati=dati+"TANGGAL AWAL tidak boleh kosong\n";	}
			if( tanggal_akhir ==""){	dati=dati+"TANGGAL AKHIR tidak boleh kosong\n";	}
			if( jabatan ==""){	dati=dati+"JABATAN tidak boleh kosong\n";	}
	if( dati !=""){
		alert(dati);
		return false;
	} else {return data;}

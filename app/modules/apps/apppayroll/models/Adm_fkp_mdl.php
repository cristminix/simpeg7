...
class Adm_Fkp_Mdl extends Apppayroll_Frontmdl {

    public $payslip_mdl_inst              = 'payslip_mdl';
    public $tbl                           = 'apr_empl_fkp';
    ...
    public $rs_use_form_filter            = 'apr_empl_fkp';
...
//        }
        ...
        if ($use_filter) {
            $this->db->where('id_pegawai NOT IN (SELECT `empl_id` FROM `apr_empl_fkp` WHERE `active_status`=1) ', null, false);
        }
        $rs = $this->db->get();
        if ($rs->num_rows() <= 0) {
            return array();
    ...
    public function add_new_member($empl_id, $fkp_id, $member_pos, $member_since, $member_term) {
...
//                        'a_class' => 'btn btn-sm btn-primary',
    ...
    public function update_member($edit_id, $fkp_id, $member_pos, $member_since, $member_term) {

...
SQL;
    ...
    public static function get_update_marstatBak($tbl, $tbl_join, $year, $month, $lastdate) {
//        return <<<SQL
//        UPDATE {$tbl} r
//            INNER JOIN (
//                SELECT
        ...
        return <<<SQL
                UPDATE {$tbl} r
                    INNER JOIN (
                        ...
                        FROM {$tbl_join} tj
                        WHERE
                        tj.print_dt='{$year}-{$month}-{$lastdate}'
                    ) 
                     ab
                    ON r.empl_id = ab.empl_id AND ab.print_dt=r.print_dt
                    SET
                        r.mar_stat = ab.mar_stat,
                        r.alw_rc_sp_cnt = ab.alw_rc_sp_cnt,
                        r.modified = NOW()

...
class Adm_Dw_Ddc_Mdl extends Apppayroll_Frontmdl {

    public $payslip_mdl_inst     = 'payslip_mdl';
    public $tbl                  = 'apr_sv_payslip';
    public $tbl_dw               = 'apr_empl_dw';
...
//    }
    ...
    public function fetch_all_member() {
        ...
        $this->db->order_by('apr_empl_dw.nipp + 0');
        $this->db->order_by('apr_empl_dw.name');
        $rs = $this->db->get();
        if ($rs->num_rows() <= 0) {
            return array();
        }
...
//        $db = $this->load->database('default', true);
    ...
    public function get_rs_action() {
        ...
        $dl_title .= lang('Download Deduction Template');
        $dl_title .= ' ';
        ...
        $im_title .= lang('Import Deduction');
        $im_title .= ' ';
        ...
        $action = array(
            ...
            'view_data' => array(
                ...
                'action_top'  => array(
                    'dl' => array(
                        'url'  => sprintf($r_url, md5('dl-ddc-tpl' . date('ymd'))),
                        'text' => $dl_title,
                    ),
                    'im' => array(
...
//$this->db->update_batch('mytable', $data, 'title');
    ...
    public function update_ddc($id, $ddc_dw) {
        $now = date('Y-m-d H:i:s');
        $this->db->set('ddc_dw', $ddc_dw);
    ...
    public function set_joins() {
        $joins          = array(
            array(
                'apr_empl_dw',
                'apr_empl_dw.empl_id=' . $this->tbl . '.empl_id AND apr_empl_dw.active_status=1',
                'left'
            )
        );
        $this->rs_joins = $joins;
    ...
    public function set_rs_select() {
        $this->rs_select = $this->tbl . '.*, apr_empl_dw.dw_id, apr_empl_dw.member_pos';
    }

    public function set_rs_select_detail() {
        $this->rs_select = $this->tbl . '.*,apr_sv_payslip.empl_name name, apr_empl_dw.dw_id, apr_empl_dw.member_pos, apr_empl_dw.member_since, apr_empl_dw.member_term';
    }

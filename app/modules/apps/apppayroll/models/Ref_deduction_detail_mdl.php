...
class Ref_Deduction_Detail_Mdl extends Apppayroll_Frontmdl {

    public $tbl               = 'apr_deduction_var';
    public $rs_index_where    = 'active_status=1';
    public $rs_deduction_data = null;

    public function add_new_deduction_var($effective_date, $variable_name, $value, $unit, $apr_deduction_id) {
        $this->db->set('effective_date', $effective_date);
        $this->db->set('variable_name', $variable_name);
        $this->db->set('value', $value);
        $this->db->set('unit', $unit);
        $this->db->set('apr_deduction_id', $apr_deduction_id);
            ...
            'view_data' => array(
                ...
                'action_top'  => array(
                    ...
                    'update'    => array(
                        ...
                        'text' => '<span class="fa fa-upload fa-border fa-fw"></span> ' . lang('Update Payslip'). ' ('.$this->rs_deduction_data->payslip_update_dt.')',
                    )
                ),
                'action_list' => array(
//                    'r' => array(
...
//                    ),
    ...
    public function update_deduction_var($id, $effective_date, $variable_name, $value, $unit) {
        $this->db->set('effective_date', $effective_date);
        $this->db->set('variable_name', $variable_name);
        $this->db->set('value', $value);
        $this->db->set('unit', $unit);

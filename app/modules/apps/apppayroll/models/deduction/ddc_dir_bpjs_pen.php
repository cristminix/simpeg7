...
class Ddc_Dir_Bpjs_Pen extends Apppayroll_Frontmdl {
    ...
    public function get_dir_bpjs_pen() {
        $res = array();
        $ls  = $this->get_ddc();
        if (!$ls) {
            return $res;
    ...
    public function update_payslip_bpjs_pen($eff_date, $val, $var) {
        $vars = array(
            ...
            'maxval' => 'bpjs_pen_max',
            'maxage' => 'bpjs_pen_maxage',
        );
        if(!isset($vars[$var])) {
            return 0;
        }
        $this->db->set($vars[$var], $val, false);
        $where = "print_dt >= '{$eff_date}' AND `lock`=0  AND (`empl_gr` = 'Direksi' OR `empl_stat` = 'Kontrak')";

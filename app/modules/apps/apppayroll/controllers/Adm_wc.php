...
class Adm_Wc extends Apppayroll_Frontctl {

    public $loan_mdl = "adm_wc_loan_mdl";
    public $main_mdl = "adm_wc_mdl";
    ...
    protected function _do_add_new_form($mdl, $empl_id) {
        ...
        $this->set_page_title(lang("Worker Cooperative") . ': ' . lang('Add new'));
    ...
    protected function _do_add_new_form_post($mdl, $empl_id, $input) {
        ...
        if (!$error) {
            ...
            $do_update = $this->{$mdl}->add_new_member($empl_id, $wc_id, $member_pos, $member_since, $member_term);
            if ($do_update) {
    ...
    protected function _do_add_new($mdl, $cur_page = 1, $per_page = 10, $order_by = null, $sort_order = 'asc', $id = null) {
        ...
        $this->{$mdl}->rs_index_where_unsigned    = 'id_pegawai NOT IN (SELECT `empl_id` FROM `apr_empl_wc` WHERE `active_status`=1)';
        ...
        $this->set_page_title(lang("Worker Cooperative Add New Member"));
    ...
    protected function _do_edit($mdl, $id) {
        ...
        $data = compact('rs_form_input', 'detail');
        $this->set_data($data);
        $this->set_page_title(lang("Worker Cooperative Edit"));
        return $this->print_page($tpl);
    }

    protected function _do_edit_loan($mdl, $id) {
        $tpl   = 'edit_detail_loan';
        $this->set_data(array('use_zebra_datepicker' => true));
        $error = array();
        $post  = $this->input->post();
        if ($post) {
            $error = $this->_do_edit_loan_post($mdl, $id, $post);
        }
        $this->{$mdl}->rs_index_where = "active_status=1";
        $this->{$mdl}->set_joins();
        ...
        $data = compact('rs_form_input', 'detail');
        $this->set_data($data);
        $this->set_page_title(lang("Worker Cooperative Deduction Edit"));
        return $this->print_page($tpl);
    }
    ...
    protected function _do_edit_loan_post($mdl, $id, $input) {
        ...
        if (!$error) {
            ...
            $do_update = $this->{$mdl}->update_loan($edit_id, $ddc_wcl);
            if ($do_update) {
                $flash_message['success'] = lang('Data has been saved');
                $this->session->set_userdata('flash_message', $flash_message);
                $r_url                    = base_url($this->router->fetch_module() . '/' . $this->router->fetch_class() . '/' . $this->router->fetch_method() . '/%s');
...
//                return $error;
        ...
        $data = compact('back_url', 'detail');
        $this->set_data($data);
        $this->set_page_title(lang("Worker Cooperative") . ' : ' . lang('Delete Confirmation'));
        return $this->print_page($tpl);
    }
    ...
    public function _download_loan_template() {
        $mdl                          = $this->loan_mdl;
        $this->load_mdl($mdl);
        $this->set_custom_filter($mdl);
        $cf                           = $this->{$mdl}->get_custom_filter_data();
        $this->{$mdl}->rs_index_where = "empl_wc=1";
        $this->{$mdl}->rs_select      = "apr_empl_wc.empl_id, apr_empl_wc.nipp, apr_empl_wc.name";
        $this->{$mdl}->set_joins();
        $list                         = $this->{$mdl}->fetch_all_member();
        $this->load->library('pr_phpexcel');
        $this->pr_phpexcel->_init();
        $sheet_title                  = 'WCL-' . $cf['cf_cur_year'] . '-' . $cf['cf_cur_month'];
    ...
    protected function _loan_import($mdl) {
        $tpl = 'loan_import';
        ...
        if ($post) {
            if (isset($post[md5('confirm-upload')])) {
                $error = $this->_loan_import_post_confirm($mdl, $post);
            } else {
                $error = $this->_loan_import_post($mdl, $post);
            }
        ...
        $this->set_data($data);
        $this->set_page_title(lang("Worker Cooperative Loan Data Import"));
        return $this->print_page($tpl);
    }

    protected function _loan_import_post($mdl, $input) {
        $this->load->library('pr_phpexcel');
        $error         = array();
        $flash_message = array();
        $field_name    = md5('file_import_loan');
        ...
        if ($x_year != $cf_cur_year && $x_month != $cf_cur_month) {
            ...
            $this->session->set_userdata('flash_message', $flash_message);
            $error['file_import_loan'] = sprintf('%s-%s', $x_year, $x_month);
            return $error;
        }
    ...
    protected function _loan_import_post_confirm($mdl, $input) {
        $error         = array();
        $flash_message = array();
        $list          = array(
            'empl_id',
...
//        $detail = array();
    ...
    public function loan($id = null, $cur_page = null, $per_page = 10, $order_by = null, $sort_order = 'asc') {
        ...
        if ($id) {
            if ($id == md5('dl-loan-tpl' . date('ymd'))) {
                return $this->_download_loan_template($mdl);
            }
            if ($id == md5('import' . date('ymd'))) {
                return $this->_loan_import($mdl);
            }
            return $this->_do_edit_loan($mdl, $id);
        }
        ...
        $this->set_page_title(lang('Worker Cooperative Loan Administration') . sprintf(' %s', $period));

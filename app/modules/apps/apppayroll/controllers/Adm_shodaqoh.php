...
class Adm_Shodaqoh extends Apppayroll_Frontctl {

    public $main_mdl  = "adm_shodaqoh_mdl";
...
//                return $error;
        ...
        $this->set_data($data);
        $this->set_page_title(lang("Shodaqoh") . ' : ' . lang('Delete Confirmation'));
        return $this->print_page($tpl);
    }
    ...
    protected function _do_del_post($mdl, $id, $input) {
        ...
        return $this->{$mdl}->delete_shodaqoh($del_id);
    }
    ...
    protected function _do_edit($mdl, $id) {
        ...
        $periode  .= ' ';
        $periode  .= $month;
        $this->set_page_title(lang("Shodaqoh Management") . ': ' . sprintf(' %s / %s - %s / %s', $periode, $detail->nipp, $detail->empl_name, $detail->job_unit));
    ...
    protected function _do_edit_post($mdl, $id, $input) {
        ...
        $var_val = filter_var($var_str, FILTER_SANITIZE_NUMBER_FLOAT);
        if (!$var_val) {
...
//        $alw_tpp_remark = filter_var($data[md5('alw_tpp_remark')], FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        ...
        if (!$error) {
            ...
            $do_update = $this->{$mdl}->update_shodaqoh($edit_id, $ddc_shd);
            if ($do_update) {
    ...
    public function _download_shodaqoh_template() {
        $mdl                          = $this->main_mdl;
        $this->load_mdl($mdl);
        $this->set_custom_filter($mdl);
        $cf                           = $this->{$mdl}->get_custom_filter_data();
        $this->{$mdl}->rs_index_where = "`lock`=0";
        $this->{$mdl}->rs_select      = "empl_id, nipp, empl_name, ddc_shd";
//        $this->{$mdl}->set_joins();
        $list                         = $this->{$mdl}->fetch_all_shodaqoh();
        $this->load->library('pr_phpexcel');
        $this->pr_phpexcel->_init();
        $sheet_title                  = 'SHD-' . $cf['cf_cur_year'] . '-' . $cf['cf_cur_month'];
        ...
        //
        $fn = 'shodaqoh' . $cf['cf_cur_year'] . $cf['cf_cur_month'];
        if (!$list) {
    ...
    protected function _shodaqoh_import($mdl) {
        $tpl = 'shodaqoh_import';
        ...
        if ($post) {
            if (isset($post[md5('confirm-upload')])) {
                $error = $this->_shodaqoh_import_post_confirm($mdl, $post);
            } else {
                $error = $this->_shodaqoh_import_post($mdl, $post);
            }
        ...
        $this->set_data($data);
        $this->set_page_title(lang("Shodaqoh Data Import"));
        return $this->print_page($tpl);
    }

    protected function _shodaqoh_import_post($mdl, $input) {
        $this->load->library('pr_phpexcel');
        $error         = array();
        $flash_message = array();
        $fieldname     = 'file_import_shodaqoh';
        $field_name    = md5($fieldname);
        $_file         = isset($_FILES[$field_name]) ? $_FILES[$field_name] : '';

        if (!isset($_file['tmp_name'])) {
            $flash_message['error'] = lang('Missing input');
            $this->session->set_userdata('flash_message', $flash_message);
            $error[$fieldname]      = lang('Required');
            return $error;
        }
        if (!$this->pr_phpexcel->is_valid_meta($_file['type'])) {
            $flash_message['error'] = lang('Invalid file type');
            $this->session->set_userdata('flash_message', $flash_message);
            $error[$fieldname]      = $_file['type'];
            return $error;
        }
        ...
        if ($x_year != $cf_cur_year && $x_month != $cf_cur_month) {
            ...
            $this->session->set_userdata('flash_message', $flash_message);
            $error[$fieldname]      = sprintf('%s-%s', $x_year, $x_month);
            return $error;
        }
        ...
        if (!$ls) {
            $flash_message['error'] = lang('Shodaqoh are empty');
            $this->session->set_userdata('flash_message', $flash_message);
            $error[]                = true;
            return $error;
        }
    ...
    protected function _shodaqoh_import_post_confirm($mdl, $input) {
        $error         = array();
        $flash_message = array();
        $list          = array(
            'empl_id',
        ...
        $do_update = $this->{$mdl}->update_batch_shodaqoh($data, 'id');
        if ($do_update) {
    ...
    public function edit($id = null, $cur_page = 1, $per_page = 10, $order_by = null, $sort_order = 'asc') {
        ...
        if ($id) {
            ...
            if ($id == md5('dl-shodaqoh-tpl' . date('ymd'))) {
                return $this->_download_shodaqoh_template($mdl);
            }
            if ($id == md5('import' . date('ymd'))) {
                return $this->_shodaqoh_import($mdl);
            }
...
//        $this->{$mdl}->set_joins();
        ...
        $this->set_page_title(lang('Shodaqoh Management') . sprintf(' %s', $period));
    ...
    public function index($id = null, $cur_page = 1, $per_page = 10, $order_by = null, $sort_order = 'asc') {
        ...
        $period = $this->_set_ym_period($this->{$mdl}->rs_cf_cur_year, $this->{$mdl}->rs_cf_cur_month);
        $this->set_page_title(lang('Shodaqoh List') . sprintf(' %s', $period));
        $this->set_pagination($mdl);

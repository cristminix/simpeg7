                        ...
                        foreach ($rs_confirm_list as $i => $row):
                            ?><tr>
                                ...
                                <td><?php echo $row['name']; ?>
                                    <input type="hidden" name="<?php echo md5('name'); ?>[]" value="<?php echo $row['name']; ?>"></td>
                                <td align="right"><?php
                                    $text = lang('#CURRENCY_SYMBOL') . " " . number_format($row['ddc'], 2, ",", ".");
                                    echo $text;
                                    ?>
                                    <input type="hidden" name="<?php echo md5('ddc_dw'); ?>[]" value="<?php echo $row['ddc']; ?>"></td>
                                    <?php
                                    ?></tr><?php
                                $n++;
                            endforeach;
...
<div class="users form">
    ...
    <form class="form-horizontal" role="form" id="UserAddForm" method="post" accept-charset="utf-8" enctype="multipart/form-data">
        ...
        <fieldset>
            <?php
            $field_alias = md5('file_import_ddc');
            $input       = array(

...
class Pns extends MX_Controller {
	...
	function view($id_pegawai=false){
		...
		$content['data'] = Modules::run("datamodel/pegawai/get_peg_pns",$id_pegawai);
		$this->load->view('pns/default',$content);
	}
	function save($id_pegawai=false,$data=array()){
		$result = Modules::run("datamodel/pegawai/set_peg_pns",$id_pegawai,$data);
		if($result)
		{

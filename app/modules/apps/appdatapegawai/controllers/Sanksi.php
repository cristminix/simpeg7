...
class Sanksi extends MX_Controller {
    ...
    $this->load->view('sanksi/default',$content);
    ...
    $content['data'] = $this->db->get('m_unor')->result();
		...
		$this->load->view('sanksi/pickerunor_data',$content);
	}
    ...
    $content['data'] = $this->db->get('m_jf')->result();
		...
		$this->load->view('sanksi/pickerjabatan_data',$content);
	}
	function submit(){
		...
		if($result)
    {
      ...
      $this->db->set('action_name','Modul Sanksi Pegawai');

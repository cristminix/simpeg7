...
class Capeg_ro extends MX_Controller {
	...
	function view($id_pegawai=false){
		...
		$content['data'] = Modules::run("datamodel/pegawai/get_peg_capeg",$id_pegawai);
		$this->load->view('capeg/default_ro',$content);
	}
	function save($id_pegawai=false,$data=array()){
		$result = Modules::run("datamodel/pegawai/set_peg_capeg",$id_pegawai,$data);
		if($result)
		{

...
class Kepangkatan_ro extends MX_Controller {
	...
	function view($id_pegawai=false,$content=array()){
		...
		$content['data'] = Modules::run("datamodel/pegawai/get_riwayat_pangkat",$id_pegawai);
		$content['panel_body'] = $this->load->view('kepangkatan/panel_body_ro',$content,true);
		$content['datalama'] = $this->getdatalama($id_pegawai);


		$this->load->view('kepangkatan/default_ro',$content);
	}
	...
	function form($id_pegawai=false,$id_peg_golongan=false){
		...
		$this->load->view('kepangkatan/form',$content);
	}

...
function ajukan(){
        ...
        success:function(data){
			location.href = '<?=site_url();?>'+'admin/module/appskp/skp_kelola';
		},
        dataType:"html"});
	return false;
}
function setForm(tujuan,idd,no){
	...
	kop['form_acc'] = "FORM PERSETUJUAN TARGET SKP dan PENGAJUAN KEPADA VERIFIKATUR"; 
	var act = []; 
	act['form_kembalikan'] = "<?=site_url();?>appskp/skp_kelola/kembali_aksi"; 
	act['form_acc'] = "<?=site_url();?>appskp/skp_kelola/acc_penilai_aksi"; 
	var btt = []; 
	...
	btt['form_acc'] = "<button id='btAct' type=sumbit class='btn btn-primary' onclick='ajukan(); return false;'><i class='fa fa-upload fa-fw'></i> Setuju dan Ajukan</button>"; 
			$.ajax({
			type:"POST",
			url:"<?=site_url();?>appskp/skp_kelola/"+tujuan,
			data:{"idd": <?=$id_skp;?> },
			beforeSend:function(){	
				$("#pageKonten").hide();
...
<div class="row">
	<div class="col-lg-12" style="padding-bottom:5px; text-align:right">
		 <a href="<?=site_url('admin/module/appskp/skp_kelola');?>"><button class="btn btn-primary" type="button"><i class="fa fa-fast-backward fa-fw"></i>Kembali...</button></a>
	</div>
	<!-- /.col-lg-12 -->
</div>
<!-- /.row -->
<div class="row">
	...
	<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenu1" data-toggle="dropdown"><span class="fa fa-tasks fa-fw"></span></button>
		<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenu1">
			...
			<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm('form_acc','1','1');"><i class="fa fa-upload fa-fw"></i>Disetujui, Ajukan kepada Verifikatur</a></li>
		</ul>
	</div>
	</div>
...
function bsmShow(tujuan,idd,idx){
	...
	tutup_track();
		...
		if(tujuan=="skp/formkomentar")
		{
			...
			data:{"idd": idd },
			beforeSend:function(){	
				$('<tr id="row_tt" class="warning"><td colspan=10><p class="text-center"><i class="fa fa-spinner fa-spin fa-1x"></i><p></td></tr>').insertAfter('#row_'+idd);
...
function kembali(){
			$.ajax({
			type:"POST",
			url:"<?=site_url();?>appskp/skp_kelola/kembali_aksi",
			data:{"komentar": "saya" },
			beforeSend:function(){	

...
<div id="gridRubrikartikel">
	<div id="rubrik-picker" class="toolbar">
		<button id="bt-add" class="tombol_aksi" onclick="loadForm('formtambahrubrik','xx');">Tambah Rubrik Agenda</button>
...
<th class=gridhead width=300><b>NAMA RUBRIK AGENDA</b></th>
...
function gridpaging(hal){
			$.ajax({
				...
				data:{"hal": hal, "batas": 10, "komponen":"agenda" },
				beforeSend:function(){	loadDialogBuka(); },
				success:function(data){
...
//tombol aksi-->
				...
				table = table+ "<div class=grid_icon onclick=\"loadForm('formeditrubrik','"+ item.id_kategori +"');\" title='Klik untuk mengedit data'><span class='ui-icon ui-icon-pencil'></span></div>";
				if(item.cek=="kosong"){
						table = table+ "<div class=grid_icon onclick=\"loadForm('formhapusrubrik','"+ item.id_kategori +"');\" title='Klik untuk menghapus data'><span class='ui-icon ui-icon-trash'></span></div>";
				}
...
//var treegrid<--
				table = table+ "<td class=gridcell align=left>" +item.nama_kategori+"</td>";
				table = table+ "<td class=gridcell align=left>" +item.keterangan+"</td>";
				table = table+ "<td class=gridcell align=left>" +item.status+"</td>";
				table = table+ "</tr>";       
			no++;
			j++;
			}); //endeach
...
function saveStatus(idd){
			$.ajax({
				type:"POST",
				url:"<?=base_url();?>cmsagenda/rubah_status",
				data:{"idd": idd },
				success:function(data){
					 if(data=='sukses'){
						$("#list").html('');
...
function naik(idini){
	...
	$.ajax({	type:"POST",	url:"<?=base_url();?>cmsagenda/naik_aksi",	data:{"id_ini": idini,"id_lawan": id_lawan,"urutan_ini": urutan_ini,"urutan_lawan": urutan_lawan },

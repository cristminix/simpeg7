...
var rubrik = $('#rubrik').val();
			$.ajax({
				...
				data:{"hal": hal, "batas": 10, "rubrik": rubrik, "komponen":"agenda"},
				beforeSend:function(){	loadDialogBuka(); },
				success:function(data){
...
//				table = table+ "<div class=grid_icon onclick=\"loadForm('formcontent/"+ item.id_konten +"','xx');\" title='Klik untuk mengedit data'><span class='ui-icon ui-icon-pencil'></span></div>";
				table = table+ "<div class=grid_icon onclick=\"loadForm('cmsagenda/formedit','"+item.id_konten +"');\" title='Klik untuk mengedit data'><span class='ui-icon ui-icon-pencil'></span></div>";
				if(item.cek=="kosong" && item.cek2=="kosong"){
					table = table+ "<div class=grid_icon onclick=\"loadForm('cmsagenda/formhapus','"+ item.id_konten +"');\" title='Klik untuk menghapus data'><span class='ui-icon ui-icon-trash'></span></div>";
				}
//tombol aksi<--
				table = table+ "</td>";
				table = table+ "<td class=gridcell align=left><div id='user_name_"+item.id_konten+"'>" +item.judul+"</div></td>";
				if(item.cek=="ada"){
					table = table+ "<td class=gridcell align=center><div id='group_name_"+item.id_konten+"' onclick=\"loadForm('cmsartikel/formfoto','"+item.id_konten +"');\" style='cursor:pointer' title='Klik untuk mengatur gambar artikel'><img src='assets/media/file/agenda/"+item.id_konten+"/" +item.thumb+"' height=40 border=0></div></td>";
				} else {
...
<div id="gridRubrikartikel">
	<div id="rubrik-picker" class="toolbar">
		<button id="bt-add" class="tombol_aksi" onclick="loadForm('cmsagenda/formtambah','xx');">Tambah Agenda</button>

...
var rubrik = $('#rubrik').val();
			$.ajax({
				type:"POST",
				url:"<?=base_url();?>cmsgaleri/getgaleri/",
...
//tombol aksi-->
				table = table+ "<div class=grid_icon onclick=\"SaveStatusComArtikel('<?=base_url();?>cmsartikel/savestatus', {id_kategori: "+ item.id_kategori +"});\" title='Rubah Status (Naik/Tarik Kembali)'><span class='ui-icon ui-icon-transferthick-e-w'></span></div>";
				table = table+ "<div class=grid_icon onclick=\"loadForm('formedit','"+item.id_kategori +"');\" title='Klik untuk mengedit data'><span class='ui-icon ui-icon-pencil'></span></div>";
				table = table+ "<div class=grid_icon onclick=\"loadForm('formfoto','"+item.id_kategori +"');\" title='Klik untuk mengedit foto'><span class='ui-icon ui-icon-image'></span></div>";
...
//tombol aksi<--
				...
				table = table+ "<td class=gridcell align=left><div id='group_name_"+item.id_kategori+"'>" +item.id_kategori+"</div></td>";
				table = table+ "<td class=gridcell align=left><div id='group_name_"+item.id_kategori+"'>" +item.tgl_buat+"</div></td>";
...
<div id="gridRubrikartikel">
	<div id="rubrik-picker" class="toolbar">
		<button id="bt-add" class="tombol_aksi" onclick="loadForm('formtambah','xx');">Tambah Berita Foto</button>
...
<th class=gridhead width=400><b>JUDUL BERITA FOTO</b></th>
<th class=gridhead><b>KETERANGAN</b></th>
<th class=gridhead width=150><b>TANGGAL</b></th>

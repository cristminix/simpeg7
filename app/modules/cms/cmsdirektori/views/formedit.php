<div style="float:left;width:80%;">
    <form id="content-form" method="post" action="<?=site_url();?>cmsdirektori/edit_aksi" enctype="multipart/form-data">
    <div style="statussave"></div>
    <table width="100%" cellspacing="0" cellpadding="0" class="table-form-flat">
        <tr>
          <th colspan="4" style="border:none">FORM EDIT ITEM DIREKTORI</th>
        </tr>
        <tr bgcolor="#99CCCC">
          <td>Kategori</td>
...
?>
        <tr>
          <td  id='label_atribut_<?=$val->urutan;?>'><?=$val->label;?></td>
          <td colspan="3">
		  <input type="text" id='isi_atribut_<?=$urutan;?>' name="isi_atribut[]" class="ipt_text" style="width:400px;" value="<?=$val->nilai;?>" onblur="if(this.value=='') this.value='Wajib diisi';" onfocus="if(this.value=='Wajib diisi') this.value='';">
...
function validasi_pengikut(opsi){
	...
	var dati="";
		...
		$("[id^='no_atribut_']").each(function(index,item) {
			var idx = item.value;
			var jdl_nm = $("#nama_atribut_"+idx+"").html();
